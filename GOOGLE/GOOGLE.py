...
class Trial(object):
    def __init__(self):
        ...
        if self.train_or_test == 1:
            ...
            self.trial_type = [3, 2, 1, 2, 2, 1, 1, 1, 1, 1, 1, 1,
                           2, 2, 1, 1, 1, 2, 2, 2, 2, 2, 2, 1]        # Trial Type = 3: Probe (Fake Hint Trial)
...
# Phase 3
    def run_hint_button(self):
        ...
        if self.trial_type[self.trial_within_block] == 1:                               # If this is a standard trial
        ...
        elif self.trial_type[self.trial_within_block] == 3:                               # If this is a probe trial
            screen.fill(blue)
            self.draw_hint_button()                                                         # Just keep the screen blue for the remaining 5 seconds until the toal_delay is reached
            cursor.draw(screen)
            moveCursor(cursor)
            if self.time_delay() >= self.total_delay:
                cursor.mv2pos((400, 300))
            ...
            if cursor.collides_with(self.stimuli[5]):                                   # If the hint button is selected, ...
                screen.fill(blue)                                                       # because it is a probe trial, skip to phase 5 and do not draw the target stimuli at the top of the screen
                self.hint_used = True
                self.phase3 = False
                self.pre_trial_delay()
                self.phase5 = True
